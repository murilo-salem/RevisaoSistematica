...
class CoordinatorAgent:
    ...
    def run(
        ...
        min_elig = self.cfg.get("routing", {}).get("min_eligibility_score", 0.2)
        mapped_tags = [
            t for t in mapped_tags
            if getattr(t, "eligibility_score", 1.0) >= min_elig
        ]
        ...
        # ---- Phase 2.5: Validation / guardrail artifacts ---------- #
        try:
            from utils import load_json
            vr = load_json("data/processed/validation_report.json")
            if isinstance(vr, dict):
                self.bb.validation_report = vr
                logger.info(
                    "Loaded validation report: %d high flags",
                    vr.get("summary", {}).get("high", 0),
                )
        except Exception:
            self.bb.validation_report = {"summary": {"total_flags": 0, "high": 0}, "flags": []}

        if not self.bb.validation_report.get("summary"):
            try:
                from paper_profiler import profile_and_validate
                _, vr = profile_and_validate(studies, mapped_chunks, self.cfg)
                self.bb.validation_report = vr
                logger.info(
                    "Validation phase executed in coordinator: %d flags",
                    vr.get("summary", {}).get("total_flags", 0),
                )
            except Exception as exc:
                logger.warning("Could not run validation phase in coordinator: %s", exc)
        self._save_blackboard()
        ...
        result = self._dispatch("formatting", Message(
            ...
            payload={
                ...
                "validation_report": self.bb.validation_report,
            },
            source="coordinator",
        ))
        if result.success:
