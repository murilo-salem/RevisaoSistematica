...
class ReviewAgent(BaseAgent):
    ...
    def process(self, message: Message) -> AgentResult:
        ...
        entry = message.payload.get("entry", {}) or {}
        validation_report = message.payload.get("validation_report", {}) or {}
        ...
        if not section_text.strip():
            return AgentResult(
                ...
                data={
                    ...
                    "cross_topic_contamination_score": 1.0,
        ...
        # Deterministic quality gates (routing/physics/grounding/table integrity)
        try:
            from quality_eval import evaluate_section
            deterministic = evaluate_section(
                section_id=section_id,
            ...
            self.logger.debug("Deterministic quality eval failed: %s", exc)
            deterministic = {
                "cross_topic_contamination_score": 1.0,
                "physics_consistency_score": 1.0,
                "grounding_score": 1.0,
                "physics_high_flag_hits": 0,
                "table_corruption_tokens": [],
                "forbidden_term_hits": [],
            }

        det_scores = [
            deterministic.get("cross_topic_contamination_score", 1.0),
            deterministic.get("physics_consistency_score", 1.0),
            deterministic.get("grounding_score", 1.0),
        ]
        deterministic_gate_score = 10.0 * sum(det_scores) / len(det_scores)

        # Hard physics gate by configuration.
        max_high_flags = self.cfg.get("validators", {}).get("max_high_flags_per_section", 0)
        high_hits = int(deterministic.get("physics_high_flag_hits", 0))
        if high_hits > max_high_flags:
            deterministic_gate_score = min(deterministic_gate_score, 5.0)
            report.overall_feedback = (
                report.overall_feedback
                + "\n- Guardrail físico: seção contém flags HIGH de consistência de fase/condição."
            ).strip()

        # Blend model score with deterministic gate (guardrails have material weight).
        report.score = round(0.7 * report.score + 0.3 * deterministic_gate_score, 2)
        ...
        return AgentResult(
            ...
            data={
                ...
                "cross_topic_contamination_score": deterministic.get("cross_topic_contamination_score", 1.0),
                "physics_consistency_score": deterministic.get("physics_consistency_score", 1.0),
                "grounding_score": deterministic.get("grounding_score", 1.0),
                "physics_high_flag_hits": deterministic.get("physics_high_flag_hits", 0),
                "forbidden_term_hits": deterministic.get("forbidden_term_hits", []),
                "table_corruption_tokens": deterministic.get("table_corruption_tokens", []),
                "section_id": section_id,
            },
        )
