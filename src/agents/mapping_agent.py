...
class MappingAgent(BaseAgent):
    ...
    def process(self, message: Message) -> AgentResult:
        ...
        for tag in all_tags:
            if getattr(tag, "eligibility_score", 1.0) < self.cfg.get("routing", {}).get("min_eligibility_score", 0.2):
                continue
        ...
        for entry in entries:
            ...
            theme_evidence[key] = ThemeEvidence(
                ...
                coverage_stats={
                    ...
                    "avg_eligibility": round(
                        sum(getattr(t, "eligibility_score", 1.0) for t in top_tags) / max(len(top_tags), 1), 3,
                    ),
                },
            )
                        ...
                        if data.get("contradiction", False):
                            te.preliminary_contradictions.append(
                                ContradictionDetail(
                                    ...
                                    evidence_chunk_ids=[chunk_a.chunk_id, chunk_b.chunk_id],
                                )
                            )
