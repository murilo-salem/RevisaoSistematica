...
class WritingAgent(BaseAgent):
    ...
    def process(self, message: Message) -> AgentResult:
        ...
        top_k = self.cfg.get("review_writer", {}).get("top_k_evidence", 10)
        min_eligibility = self.cfg.get("routing", {}).get("min_eligibility_score", 0.2)
        ...
        # Add synthesis anchors when available to improve grounding.
        if isinstance(synthesis_map, dict):
            anchor_lines = []
            for cp in synthesis_map.get("consensus_points", [])[:6]:
                if isinstance(cp, dict) and cp.get("evidence_chunk_ids"):
                    anchor_lines.append(
                        f"- {cp.get('statement', '')} [chunks:{', '.join(cp.get('evidence_chunk_ids', []))}]"
                    )
            if anchor_lines:
                evidence = (
