...
def _textual_cleanup(text: str) -> str:
    ...
    # 3.1 Remove common corruption tokens from broken table extraction
    text = re.sub(r'/C\d+', '', text)
    text = re.sub(r'\bC(\d+)\s+C\b', r'\1 C', text)
...
def post_process_review(
    ...
    # ---- Deterministic quality report refresh --------------------- #
    try:
        from utils import load_json
        from quality_eval import build_quality_report

        quality_sections = [
            {"parent": sec.parent, "folder": sec.heading, "content": sec.body}
            for sec in merged
            if sec.level == 3
        ]

        taxonomy_entries = []
        previous_section_meta: Dict[str, Dict[str, Any]] = {}
        try:
            prev_sections = load_json("data/results/review_sections.json")
            if isinstance(prev_sections, list):
                for s in prev_sections:
                    if not isinstance(s, dict):
                        continue
                    sid = f"{s.get('parent', '')} / {s.get('folder', '')}"
                    previous_section_meta[sid] = s
                taxonomy_entries = [
                    {
                        "parent": s.get("parent", ""),
                        "folder": s.get("folder", ""),
                        "allowed_domains": s.get("allowed_domains", []),
                        "required_phase_regimes": s.get("required_phase_regimes", []),
                        "required_any_terms": s.get("required_any_terms", []),
                        "forbidden_terms": s.get("forbidden_terms", []),
                        "min_similarity": s.get("min_similarity", 0.0),
                    }
                    for s in prev_sections
        ...
        # Reattach filtering stats from stage-9 sections when available.
        for sec in quality_sections:
            sid = f"{sec.get('parent', '')} / {sec.get('folder', '')}"
            meta = previous_section_meta.get(sid, {})
            if isinstance(meta, dict):
                sec["n_evidence"] = meta.get("n_evidence", 0)
                sec["n_evidence_after_filter"] = meta.get("n_evidence_after_filter", 0)
                sec["filter_stats"] = meta.get("filter_stats", {})
        ...
        logger.debug("Could not refresh quality report in post-processing: %s", exc)
