...
def _normalise_outline_entry(entry: Dict[str, Any]) -> Dict[str, Any]:
    """Ensure outline entries contain guardrail fields with safe defaults."""
    norm = dict(entry)
    norm.setdefault("allowed_domains", [])
    norm.setdefault("required_phase_regimes", [])
    norm.setdefault("required_any_terms", [])
    norm.setdefault("forbidden_terms", [])
    try:
        norm["min_similarity"] = float(norm.get("min_similarity", 0.0) or 0.0)
    except Exception:
        norm["min_similarity"] = 0.0
    try:
        norm["max_foreign_domain_ratio"] = float(
            norm.get("max_foreign_domain_ratio", 1.0) or 1.0
        )
    except Exception:
        norm["max_foreign_domain_ratio"] = 1.0
    norm["max_foreign_domain_ratio"] = max(0.0, min(1.0, norm["max_foreign_domain_ratio"]))
    try:
        norm["max_foreign_evidence_share"] = float(
            norm.get("max_foreign_evidence_share", 1.0) or 1.0
        )
    except Exception:
        norm["max_foreign_evidence_share"] = 1.0
    norm["max_foreign_evidence_share"] = max(0.0, min(1.0, norm["max_foreign_evidence_share"]))
    strictness = str(norm.get("routing_strictness", "soft")).lower()
    norm["routing_strictness"] = strictness if strictness in {"soft", "hard"} else "soft"
    return norm
            ...
            with open(tax_path, "r", encoding="utf-8") as fh:
                ...
                if isinstance(data, list):
                    ...
                    if data and isinstance(data[0], dict) and "prompt" in data[0]:
                        ...
                        entries = [_normalise_outline_entry(item) for item in data]
                ...
                if isinstance(data, dict) and data.get("type") == "outline":
                    entries = data.get("entries", [])
                    if isinstance(entries, list):
                        data["entries"] = [
                            _normalise_outline_entry(e)
                            for e in entries
