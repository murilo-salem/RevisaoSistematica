...
class NumericRange(BaseModel):
    """Numeric range with confidence metadata."""
    min: Optional[float] = None
    max: Optional[float] = None
    confidence: float = 0.0


class AnchoredClaim(BaseModel):
    """Claim grounded in one or more evidence chunks."""
    claim: str = ""
    claim_type: Literal["quant", "qual"] = "qual"
    evidence_chunk_ids: List[str] = Field(default_factory=list)


class ValidationFlag(BaseModel):
    """Deterministic validation issue produced by validators."""
    flag_type: str = ""
...
class PaperProfile(BaseModel):
    """Canonical structured profile for one paper/study."""
    paper_id: str
    domains: List[str] = Field(default_factory=list)
    working_fluids: List[str] = Field(default_factory=list)
    phase_regimes: List[str] = Field(default_factory=list)
    temperature_range_K: NumericRange = Field(default_factory=NumericRange)
    pressure_range_bar: NumericRange = Field(default_factory=NumericRange)
    system_topologies: List[str] = Field(default_factory=list)
    keywords: List[str] = Field(default_factory=list)
    anchored_claims: List[AnchoredClaim] = Field(default_factory=list)
...
class TaxonomyEntry(BaseModel):
    ...
    required_phase_regimes: List[str] = Field(default_factory=list)
    required_any_terms: List[str] = Field(default_factory=list)
    forbidden_terms: List[str] = Field(default_factory=list)
    min_similarity: float = 0.0
    max_foreign_domain_ratio: float = 1.0
    max_foreign_evidence_share: float = 1.0
    routing_strictness: Literal["soft", "hard"] = "soft"
